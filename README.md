...
### V4.5.7 GI Bill hover migration fix

- Ensures the green earned-benefit section always shows a visible `See calculation` control for military paths.
- Restores the full hover calculation for earlier saved paths when the current GI Bill inputs reproduce the saved total.
- Explains when a legacy path must be rebuilt because its older saved record contains only the total and not the underlying tuition, housing, books, years, and eligibility inputs.
...
- Retains the calculation in the military builder and beneath the green earned-benefit section of the printable decision plan.

### V4.5.4 decision-plan GI Bill explanation

- Adds the full tuition, housing, books, eligibility, and total arithmetic directly beneath the GI Bill value in the printable decision plan.
- Keeps the calculation visible when the plan is printed or saved as a PDF so families can understand the estimate without reopening the builder.

### V4.5.3 transparent GI Bill calculation

- Shows the exact tuition, housing, books, academic years, benefit months, and estimated eligibility percentage used to calculate the Post-9/11 GI Bill value.
- Carries the calculation breakdown into the saved military path so the total is explainable in comparison cards and downstream analysis data.
- Clearly separates the estimated future education benefit from immediate military cash compensation and retains the requirement to verify actual VA eligibility and enrollment.
...
### V4.4.16 streamlined path to the builder
